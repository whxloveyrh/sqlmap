                ...
                for where in test.where:
                    ...
                    for method, check in test.response.items():
                        ...
                        # SQL injections
                            ...
                            '''
                            其中，重点注意Request.queryPage函数，将参数timeBasedCompare设置为True，
                            所以在Request.queryPage函数内部,有这么一段代码：
                            if timeBasedCompare:
                                return wasLastRequestDelayed()
                            而函数wasLastRequestDelayed()的功能主要是判断最后一次的请求是否有明显的延时，
                            方法就是将最后一次请求的响应时间与之前所有请求的响应时间的平均值进行比较，
                            如果最后一次请求的响应时间明显大于之前几次请求的响应时间的平均值，就说明有延迟。
                            '''
                            trueResult = Request.queryPage(reqPayload, place, timeBasedCompare=True, raise404=False)

                            '''
                            trueResult=true表示，此URL可能是基于时间的盲注，需要再次确认，避免出现误判
                            trueResult=false表示此URL不能基于时间的盲注
                            为了确保基于时间的盲注的准确性，sqlmap执行了两次queryPage。
                            ...
                            if trueResult:
                                ...
                                '''
                                两次测试结果均是Ture,表明此URL是可以基于时间的盲注。否则，表示不能基于时间的盲注
                                '''
                                if trueResult:
                                    infoMsg = "%s parameter '%s' appears to be '%s' injectable " % (paramType, parameter, title)
                                    logger.info(infoMsg)
...
def heuristicCheckSqlInjection(place, parameter):
    '''
        ...
        首先会从HEURISTIC_CHECK_ALPHABET中随机抽取10个字符出现构造Payload，当然里面的都不是些普通的字符，
        而且些特殊字符，当我们进行SQL注入测试的时候会很习惯的在参数后面加个分号啊什么的，
        又或者是其他一些特殊的字符，出现运气好的话有可能会暴出数据的相关错误信息，
        而那个时候我们就可以根据所暴出的相关错误信息去猜测当前目标的数据库是什么。
        '''
    ...
    paramType = conf.method if conf.method not in (None, HTTPMETHOD.GET, HTTPMETHOD.POST) else place
    prefix = ""  #存储sqlmap生成的前缀信息,每次攻击的前缀都不一样
    suffix = ""  #存储sqlmap生成的后缀信息,每次攻击的后缀都不一样
    ...
    '''
    if value.lower() in (page or "").lower():   #存在XSS攻击
...
def checkConnection(suppressOutput=False):
    if not any((conf.proxy, conf.tor, conf.dummy, conf.offline)):
        try:
            ...
            logger.debug(debugMsg)
            socket.getaddrinfo(conf.hostname, None)  #getaddrinfo可解析得到IPv6地址，而gethostbyname仅能得到IPv4地址。getaddrinfo在Python的socket包中
