...
class Connect(object):
    ...
    def queryPage(value=None, place=None, content=False, getRatioValue=False, silent=False, method=None, timeBasedCompare=False, noteResponseTime=True, auxHeaders=None, response=False, raise404=None, removeReflection=True):
        ...
        if payload:
            if kb.tamperFunctions: #tamper脚本在queryPage函数中被调用，queryPage函数是用来请求页面内容，在每次发送请求之前，先会将payload进行tamper函数处理。
        ...
        if timeBasedCompare:  #当进行基于时间的盲注，直接返回执行结果
            '''
            ...
            若wasLastResponseDelayed()函数返回值为true，表示可能是基于时间的盲注。否则，表示不是基于时间的盲注
            '''
